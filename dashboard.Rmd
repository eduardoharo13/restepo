...
b <- reactive({as.data.frame(sort(table(cap200[which(cap200$NOMBREDI==input$Distrito),]$IH203_MES,cap200[which(cap200$NOMBREDI==input$Distrito),]$IH208_GENERICO ),decreasing = T))})
 ...
 renderPlot({ ggplot(as.data.frame(b()), aes(x=Var1,y = Freq,group = Var2 ))+
  ...
  geom_label(aes(label=b()$Freq),size = 3, fill = "white")+
  xlab("Mes") +   
